...
public class CommandsConfigurer : ICommandsConfigurer
{
    ...
    public void Configure(IConfigurator cmd)
    {
        ...
        {
            entry.SetDescription("Gerencia as marcações no registro.");

            entry.AddCommand<EntryNewCommand>("new")
                .WithDescription("Registra novas marcações.")
            ...
            entry.AddCommand<EntryOverrideCommand>("override")
                .WithDescription("Sobrescreve marcações no registro.")
            ...
            entry.AddCommand<EntryNowCommand>("now")
                .WithDescription("Escreve uma nova marcação no registro agora.");
        });

        cmd.AddBranch("report", report =>
        {
            report.SetDescription("Consolida e exibe as marcações no registro.");

            report.AddCommand<ReportSimulateCommand>("simulate")
                .WithDescription("Simula novas marcações para obter saldo acumulado estimado do banco de horas.")
            ...
            report.AddCommand<ReportMyDrakeCommand>("mydrake")
                .WithDescription("Exporta as marcações do registro para inclusão em lote no MyDrake.")
